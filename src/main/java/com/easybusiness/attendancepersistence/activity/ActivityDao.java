...
public class ActivityDao {
	...
	return activityRepository.findById(id).get(0);
    }
	...
	if (activityRepository.findById(activity.getId()).size() > 0) {
	    activity.setId(activityRepository.findById(activity.getId()).get(0).getId());
	}
